							...
							<?php if(count($ds_photo)!=0){?>       
								<?php for($i=0;$i<count($ds_photo);$i++){?>
									<div class="item_trich">
										<img class="img_trich" width="<?=_width_thumb_img?>" height="<?=_height_thumb_img?>" src="<?=_upload_product.$ds_photo[$i]['photo']?>" />
										<input type="text" rel="<?=$ds_photo[$i]['id']?>" data-ten="stt" value="<?=$ds_photo[$i]['stt']?>" class="update_stt" />
										<input type="text" rel="<?=$ds_photo[$i]['id']?>" data-ten="mota" value="<?=$ds_photo[$i]['mota']?>" class="update_stt" />
	...
	$(document).ready(function() {
		$('.file_input').filer({
			...
			templates: {
				...
				</div>\<input type="text" name="stthinh[]" class="stthinh" placeholder="Nhập STT" />\<input type="text" name="mota[]" class="mota" placeholder="Nhập link" />\
				</div>\
				</div>\
				</li>',
